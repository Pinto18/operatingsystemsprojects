...
/*CS 420 Assignment # 4
/*                                                                                                                  */
/*  Program to demonstrate a race condition.  In the middle of a critical section, the 'main'  */
...
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;   /* the mutex lock for this program                             */
/********************************************************************************************************************/
...
{                             /*  kernel-level thread.  In C, that has to be the one that starts executing 'main'   */
   ...
   if ( pthread_create(&new_tid_no, NULL, second_thread, NULL) == 0)    /* Create another thread; check for success */
      {
         if((errorCode = pthread_mutex_lock(&mutex)) != 0)              /******  Put your ENTRY SECTION code here.  ******/
         {
		printf("\n  Bad mutex_lock: %s", strerror(errorCode));
		return -1;
	 }
	 else
	 {
         ...
         if((errorCode = pthread_mutex_unlock(&mutex)) != 0)           /******  Put your EXIT SECTION code here.   ******/
    	 {
		printf("Bad mutex unlock: %s", strerror(errorCode));
		return -1;
	 }
...
void *second_thread(void *dummy)             /* The thread created by 'main' starts execution here.                            */
{
   ...
   if((errorCode = pthread_mutex_lock(&mutex)) != 0)
	printf("Bad mutex lock: %s", strerror(errorCode));
   else
   {
   ...
   if((errorCode = pthread_mutex_unlock(&mutex)) != 0 )
	printf("Bad mutex unlock: %s", strerror(errorCode));
   else
